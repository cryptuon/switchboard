# ChainSync Quick Start Guide

Get up and running with ChainSync in under 5 minutes! This guide will walk you through deploying your first cross-chain contract.
...
mkdir my-chainsync-app
cd my-chainsync-app

# Initialize npm project
npm init -y

# Install ChainSync SDK
...
async function quickStart() {
  console.log('🚀 ChainSync Quick Start Demo');
  console.log('================================\n');

  // Step 1: Initialize ChainSync
  console.log('📡 Initializing ChainSync...');
  ...
  console.log('✅ ChainSync initialized\n');
...
Expected output:
```
🚀 ChainSync Quick Start Demo
================================

📡 Initializing ChainSync...
✅ ChainSync initialized
...
1. **Initialized ChainSync**: Connected to Solana coordination layer and chain RPCs
...
git clone https://github.com/your-org/chainsync-examples.git
cd chainsync-examples
...
const myContract = await chainSync.deployContract({
...
async function trackWithNotifications(deploymentId: string) {
  const status = await chainSync.trackTransaction(deploymentId);
...
async function trackWithTimeout(id: string, timeoutMs = 300000) {
  ...
  while (Date.now() - startTime < timeoutMs) {
    const status = await chainSync.trackTransaction(id);
...
- **Full Documentation**: [docs.chainsync.org](https://docs.chainsync.org)
- **SDK Reference**: [SDK Documentation](../sdk/README.md)
- **More Examples**: [Examples Directory](./README.md)
- **Community**: [Discord](https://discord.gg/chainsync)
...
- 🐛 **GitHub Issues**: [Report bugs](https://github.com/your-org/chainsync/issues)
- 📧 **Email**: support@chainsync.org
- 📚 **Docs**: [docs.chainsync.org](https://docs.chainsync.org)

---

**🎉 Congratulations!** You've successfully deployed your first cross-chain contract with ChainSync. Welcome to the future of unified blockchain development!